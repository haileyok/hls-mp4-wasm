...
function App() {
  ...
  const createMp4 = React.useCallback(async (videoUrl: string) => {
    ...
    let error = false
    await Promise.all(playlist.segments.map(async (segment) => {
      try {
    ...
    await ffmpeg.exec(['-i', `concat:${segments.join('|')}`, '-c:v', 'copy', 'output.mp4'])
    
    // Create a url for the player
    const fileData = await ffmpeg.readFile('output.mp4')
    const blobUrl = URL.createObjectURL(new Blob([fileData.buffer], {type: 'video/mp4'}))
    setMp4(blobUrl)
  }, [])
